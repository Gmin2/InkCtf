          ...
          messages.map((msg) => {
            ...
            return (
              <div key={msg.id} className="flex gap-1.5 leading-tight">
                ...
                <span className={`${msg.type === 'error' || msg.type === 'warning' ? 'whitespace-pre-wrap break-all' : 'truncate'} ${isLight ? 'text-zinc-700' : 'text-[var(--text-primary)]/90'}`}>
                  {msg.type === 'error' || msg.type === 'warning' || msg.type === 'success' ? msg.message : truncateMessage(msg.message, 80)}
                </span>
              </div>
            );
          })
